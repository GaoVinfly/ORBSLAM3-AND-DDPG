...
from utils_RL11 import conv2d_layer, fully_connected_layer, stateful_lstm, a_fully_connected_layer
...
class DeepQNetwork(BaseModel):
    ...
    def A_build_net(self, S, init_state, scope):
        ...
        with tf.variable_scope(scope):
            ...
            if scope == 'a_eval_net':
                ...
                logging.info("lstm部分out的类型" + str(out))
                logging.info("lstm部分state的类型" + str(state))
                ...
                logging.info("输入全连接层out的类型" + str(out))
    ...
    def save_model(self, step=1, imgname=None):
        ...
        model_save_path = 'Model_double_GRU/'
        model_name = 'model'
    ...
    def load_model(self, imgname=None):
        # model_save_path = './Model_m1/'+imgname+'/'
        last_model_path = 'Model_double_GRU/'
        model_name = 'model'
        savers = tf.train.Saver()
