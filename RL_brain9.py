...
from utils import conv2d_layer, fully_connected_layer, stateful_lstm, a_fully_connected_layer
...
class DeepQNetwork(BaseModel):
    def __init__(
        ...
        with tf.variable_scope('soft_replacement'):
            ...
            self.target_replace_op = [tf.assign(t, (1-self.TAU) * t  + self.TAU * e) for t, e in
                                    zip(t_params + a_t_params, e_params + a_e_params)]  # assign把e赋值给t ,更新 target_net 参数
    ...
    def A_build_net(self, S, init_state, scope):
        ...
        with tf.variable_scope(scope):
            ...
            if scope == 'a_eval_net':
                ...
                # 卷积层2
                w, b, out, summary = conv2d_layer(out, self.batch_size, [4, 4], [2, 2], scope_name="conv2_train",
                ...
                # 卷积层3
                w, b, out, summary = conv2d_layer(out, self.batch_size, [3, 3], [1, 1], scope_name="conv3_train",
                ...
                w, b, out, _ = conv2d_layer(out, self.batch_size, [4, 4], [2, 2], scope_name="conv2_target", summary_tag=None,
                                            activation=tf.nn.relu, data_format=self.cnn_format)
                self.w_target_a["wc2"] = w
                self.w_target_a["bc2"] = b

                w, b, out, _ = conv2d_layer(out, self.batch_size, [3, 3], [1, 1], scope_name="conv3_target", summary_tag=None,
                                            activation=tf.nn.relu, data_format=self.cnn_format)
    ...
    # def choose_init_state(self, observation):
    ...
    # def choose_action(self, observation):
    #     self.initial_zero_state_single = self.sess.run(self.state_output_c, feed_dict={self.init_state: self.initial_zero_state_single,})
    #     s = observation
    #     return self.sess.run(self.a, feed_dict={ self.state : [[s]]})
    ...
    #     if np.random.uniform() < self.epsilon:  # rand()通过本函数可以返回一个或一组服从“0~1”均匀分布的随机样本值。随机样本取值范围是[0,1)，不包括1
    #         actions_value,self.initial_zero_state_single= self.sess.run(
    #             [self.q_out, self.state_output_c], feed_dict={self.state: [[image]],self.init_state: self.initial_zero_state_single,
    #                 # self.h_state_train: self.lstm_state_h
    #             })
    #         action = np.argmax(actions_value)
    #         return action
