...
class ReplayMemory:
    ...
    def __init__(self, config):
       ...
       #np.load和np.save 是读写磁盘数组数据的两个主要函数，默认情况下，数组是以未压缩的原始二进制格式保存在扩展名为.npy的文件中。
...
class DRQNReplayMemory(ReplayMemory):
    ...
    def __init__(self, config):
        ...
        self.states_ = np.empty((self.config.batch_size, self.config.min_history + self.config.states_to_update + 1, self.config.screen_height, self.config.screen_width), dtype=np.uint8)
    ...
    def sample_batch(self):
        ...
        while len(indices) < self.config.batch_size:
            ...
            self.states_[len(indices)] = self.getState(index)
            indices.append(index)
        return self.states, self.actions_out, self.rewards_out, self.states_
