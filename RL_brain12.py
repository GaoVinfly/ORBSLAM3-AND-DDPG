...
from utils_RL11 import conv2d_layer, fully_connected_layer, stateful_lstm, a_fully_connected_layer, out_stateful_lstm
...
class DeepQNetwork(BaseModel):
    def __init__(
        ...
        with tf.variable_scope('A_train'):
            ...
            # self.a_train = tf.train.RMSPropOptimizer(self.lr).minimize(self.a_cost)
            # self.a_train = tf.train.AdamOptimizer(1e-3).minimize(self.a_cost)
            self.a_train = tf.train.AdamOptimizer(self.lr).minimize(self.a_cost)
    ...
    def A_build_net(self, S, init_state, scope):
        ...
        with tf.variable_scope(scope):
            ...
            if scope == 'a_eval_net':
                ...
                first_lstm = tf.nn.rnn_cell.MultiRNNCell(
                    [stateful_lstm(out_flat, self.num_lstm_layers, self.lstm_size, init_state,
                                   scope_name="lstm_train_1")])
                second_lstm = tf.nn.rnn_cell.MultiRNNCell(
                    [out_stateful_lstm(out_flat, self.num_lstm_layers, self.lstm_size, init_state,
                                   scope_name="lstm_train_2")])
                first_init = first_lstm.zero_state(self.batch_size, tf.float32)
                second_init = second_lstm.zero_state(self.batch_size, tf.float32)

                out, state = tf.nn.bidirectional_dynamic_rnn(cell_fw=first_lstm, cell_bw=second_lstm,
                                                             initial_state_fw=first_init, initial_state_bw=second_init, scope = "lstm_train")
                ...
                # 全连接层
                w, b, out = a_fully_connected_layer(out, self.n_actions, scope_name="out_train", activation=tf.nn.tanh)
                self.w_a["wout"] = w
                self.w_a["bout"] = b
                self.a_eval = out
                return self.a_eval
                ...
                first_lstm = tf.nn.rnn_cell.MultiRNNCell(
                    [stateful_lstm(out_flat, self.num_lstm_layers, self.lstm_size, init_state,
                                   scope_name="lstm_target_1")])
                second_lstm = tf.nn.rnn_cell.MultiRNNCell(
                    [out_stateful_lstm(out_flat, self.num_lstm_layers, self.lstm_size, init_state,
                                   scope_name="lstm_target_2")])
                first_init = first_lstm.zero_state(self.batch_size, tf.float32)
                second_init = second_lstm.zero_state(self.batch_size, tf.float32)

                out, state = tf.nn.bidirectional_dynamic_rnn(cell_fw=first_lstm, cell_bw=second_lstm,
                                                             initial_state_fw=first_init, initial_state_bw=second_init, scope = "lstm_target")
                ...
                out = tf.reshape(out, [tf.shape(out)[0], shape[2]])
                # 全连接层
                w, b, out = a_fully_connected_layer(out, self.n_actions, scope_name="out_target", activation=tf.nn.tanh)
